...
typedef enum ccn_upcall_res (*ccn_handler)(struct ccn_closure *selfp,
		enum ccn_upcall_kind kind, struct ccn_upcall_info *info /**< details about the event */
		);
 ...
 */
int ccn_name_append_components(struct ccn_charbuf *c, const unsigned char *ccnb,
		size_t start, size_t stop);

enum ccn_marker {
	CCN_MARKER_NONE = -1, CCN_MARKER_SEQNUM = 0x00, /**< consecutive block sequence numbers */
 ...
 */
int ccn_name_append_numeric(struct ccn_charbuf *c, enum ccn_marker tag,
		uintmax_t value);
 ...
 */
int ccn_name_chop(struct ccn_charbuf *c, struct ccn_indexbuf* components,
		int n);
...
enum ccn_content_type {
	...
	/* for sending a hello message */
	CCN_CONTENT_HELLO = 0x3C148B,
	/* for sending the pending interest's table (PIT) to another node */
	CCN_CONTENT_PIT = 0x40AC15
};
...
int ccn_express_interest(struct ccn *h, struct ccn_charbuf *namebuf,
		struct ccn_closure *action, struct ccn_charbuf *interest_template);
...
int ccn_set_interest_filter_with_flags(struct ccn *h,
		struct ccn_charbuf *namebuf, struct ccn_closure *action,
		int forw_flags);
...
int ccn_get(struct ccn *h, struct ccn_charbuf *name,
		...
		struct ccn_charbuf *resultbuf, struct ccn_parsed_ContentObject *pcobuf,
...
int ccn_verify_content(struct ccn *h, const unsigned char *msg,
		struct ccn_parsed_ContentObject *pco);
...
int ccn_buf_match_blob(struct ccn_buf_decoder *d, const unsigned char **bufp,
		size_t *sizep);
...
int ccn_parse_required_tagged_BLOB(struct ccn_buf_decoder *d,
		enum ccn_dtag dtag, int minlen, int maxlen);
...
int ccn_parse_tagged_string(struct ccn_buf_decoder *d, enum ccn_dtag dtag,
...
uintmax_t ccn_parse_optional_tagged_binary_number(struct ccn_buf_decoder *d,
		enum ccn_dtag dtag, int minlen, int maxlen, uintmax_t default_value);
/**
 ...
 */
int ccn_ref_tagged_BLOB(enum ccn_dtag tt, const unsigned char *buf,
		size_t start, size_t stop, const unsigned char **presult,
		size_t *psize);
 ...
 * Returns 0 for success, negative value for error.
 */
int ccn_ref_tagged_string(enum ccn_dtag tt, const unsigned char *buf,
		size_t start, size_t stop, const unsigned char **presult,
		size_t *psize);
 ...
 */
int
ccn_parse_Link(struct ccn_buf_decoder *d, struct ccn_parsed_Link *link,
		struct ccn_indexbuf *components);
 ...
 */
int
ccnb_append_Link(struct ccn_charbuf *buf, const struct ccn_charbuf *name,
		const char *label, const struct ccn_charbuf *linkAuthenticator);
...
ccn_parse_Collection_next(struct ccn_buf_decoder *d,
		struct ccn_parsed_Link *link, struct ccn_indexbuf *components);
...
ccn_parse_interest(const unsigned char *msg, size_t size,
		struct ccn_parsed_interest *interest, struct ccn_indexbuf *components);
...
/*
 * Returns the request rate of the content object
 */
...
/*
 * Returns the delivery rate of the content object
 */
int ccn_delivery_rate(const unsigned char *msg,
		const struct ccn_parsed_ContentObject *po);

/*
 * Encode statistics into the content object
 */
int ccn_encode_statistics(struct ccn_charbuf*, int, int);
...
int ccn_parse_ContentObject(const unsigned char *msg, size_t size,
		struct ccn_parsed_ContentObject *x, struct ccn_indexbuf *components);
...
int ccn_name_comp_strcmp(const unsigned char *data,
		const struct ccn_indexbuf *indexbuf, unsigned int i, const char *val);
...
int ccn_name_comp_get(const unsigned char *data,
		const struct ccn_indexbuf *indexbuf, unsigned int i,
...
int ccn_sign_content(struct ccn *h, struct ccn_charbuf *resultbuf,
		...
		const struct ccn_signing_params *params, const void *data, size_t size);

int ccn_load_private_key(struct ccn *h, const char *keystore_path,
		const char *keystore_passphrase, struct ccn_charbuf *pubid_out);

int ccn_load_default_key(struct ccn *h, const char *keystore_path,
		const char *keystore_passphrase);

int ccn_get_public_key(struct ccn *h, const struct ccn_signing_params *params,
		struct ccn_charbuf *digest_result, struct ccn_charbuf *result);

int ccn_chk_signing_params(struct ccn *h,
		const struct ccn_signing_params *params,
		struct ccn_signing_params *result, struct ccn_charbuf **ptimestamp,
		struct ccn_charbuf **pfinalblockid, struct ccn_charbuf **pkeylocator);

/* low-level content-object signing */

#define CCN_SIGNING_DEFAULT_DIGEST_ALGORITHM "SHA256"

int ccn_signed_info_create(struct ccn_charbuf *c, /* filled with result */
...
int ccn_encode_ContentObject(struct ccn_charbuf *buf,
		const struct ccn_charbuf *Name, const struct ccn_charbuf *SignedInfo,
		const void *data, size_t size, const char *digest_algorithm,
...
int ccn_content_matches_interest(const unsigned char *content_object,
		size_t content_object_size, int implicit_content_digest,
		struct ccn_parsed_ContentObject *pc, const unsigned char *interest_msg,
		size_t interest_msg_size, const struct ccn_parsed_interest *pi);

/*
 * Test whether the given raw name is int the Exclude set.
 */
int ccn_excluded(const unsigned char *excl, size_t excl_size,
		const unsigned char *nextcomp, size_t nextcomp_size);

/***********************************
 * StatusResponse
 */
int ccn_encode_StatusResponse(struct ccn_charbuf *buf, int errcode,
		const char *errtext);
 ...
 */
int ccnb_append_timestamp_blob(struct ccn_charbuf *c, enum ccn_marker marker,
...
int ccn_resolve_version(struct ccn *h, struct ccn_charbuf *name, /* ccnb encoded */
