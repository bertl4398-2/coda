...
struct ccnd_handle {
	...
	struct hashtb *content_stats_tab; /**< FRDS: hash table with stats for each content object ever seen, keyed by name prefix compononents of query */
	...
	struct ccn_scheduled_event *hello;
	struct ccn_scheduled_event *pit;
	...
	struct ccn_charbuf *server_id; /**< nonce containing the server ID */
	...
	double stats_w_seconds; /**< CCN_STATS_W_SECONDS */
	double stats_ttl_seconds; /**< CCN_STATS_TTL_SECONDS */
	double stats_alpha; /**< CCN_STATS_ALPHA */
	double stats_beta; /**< CCN_STATS_BETA */
	enum ccnx_mode mode; /**< use base CCNx, MRU LRU, or utility optimization */
	int hello_periodicity; /**< periodicity of HELLO messages in microseconds */
	int hello_backoff; /**< for calculating HELLO message send delay*/
	int hello_max_backoff;
	int pit_delay;
	int pit_backoff;
	int pit_max_backoff;
};
...
enum ccnd_face_meter_index {
	FM_BYTI, FM_BYTO, FM_DATI, FM_INTO, FM_DATO, FM_INTI, CCND_FACE_METER_N
};
...
struct content_entry {
	...
	double utility; /**< Utility of content object */
	int req_rate;
	int delivery_rate;
};

/*
 * we do NOT want the server to receive its own multicast packets
 *//*
 #define IP_MULTICAST_LOOP 1
 #define IPV6_MULTICAST_LOOP 1
 */
/* by default content statistics expire after 300 seconds, unless an environment
 variable with the same name states another value: see ccnd_handle. */
#define CCN_STATS_W_SECONDS 300.0

/*
 * default information-request lifetime
 */
#define CCN_STATS_TTL_SECONDS 30.0

/*
 * EWMA weight for the request-rate estimator
 */
#define CCN_STATS_ALPHA 0.9

/*
 * EWMA weight for the delivery-rate estimator
 */
#define CCN_STATS_BETA 0.9
/*
 * by default HELLO messages are sent every 5 seconds
 */

#define CCN_HELLO_PERIODICITY 5000000

/*
 * URI of HELLO content object
 */

#define HELLO_MSG_URI "/coda/hello"

/*
 * URI of PIT content object
 */

#define PIT_MSG_URI "/coda/pit"

/**
 * calculate the delay for sending a hello message
 */

#define CCN_HELLO_MAX_BACKOFF 3
#define HELLO_BACKOFF_TIME(h) ((h->hello_periodicity * h->hello_backoff) + h->hello_periodicity * (rand()*1.0/(RAND_MAX + 1.0)))

/*
 * calculate the delay for sending the pit message
 */

#define CCN_PIT_DELAY CCN_HELLO_PERIODICITY
#define CCN_PIT_MAX_BACKOFF CCN_HELLO_MAX_BACKOFF
#define PIT_BACKOFF_TIME(h) ((h->pit_delay * h->pit_backoff) + h->pit_delay * (rand()*1.0/(RAND_MAX + 1.0)))

/**
 * FRDS: Contains the statistics pertaining to each content object
 * ever received.
 * key is the ccnb encoded header of a ContentObject message
 */

struct content_stats_entry {
	unsigned long number_requests;
	unsigned long satisfied_requests;
};
...
void ccnd_meter_init(struct ccnd_handle *h, struct ccnd_meter *m,
		const char *what);
...
void ccnd_debug_ccnb(struct ccnd_handle *h, int lineno, const char *msg,
		struct face *face, const unsigned char *ccnb, size_t ccnb_size);
