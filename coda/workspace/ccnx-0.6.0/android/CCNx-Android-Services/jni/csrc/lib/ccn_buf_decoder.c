...
ccn_buf_decoder_start(struct ccn_buf_decoder *d, const unsigned char *buf,
		size_t size) {
...
void ccn_buf_advance(struct ccn_buf_decoder *d) {
	ccn_skeleton_decode(&d->decoder, d->buf + d->decoder.index,
...
int ccn_parse_PublisherID(struct ccn_buf_decoder *d,
	...
	if (iskey || ccn_buf_match_dtag(d, CCN_DTAG_PublisherCertificateDigest)
			|| ccn_buf_match_dtag(d, CCN_DTAG_PublisherIssuerKeyDigest)
			|| ccn_buf_match_dtag(d,
...
int ccn_parse_interest(const unsigned char *msg, size_t size,
	...
	if (ccn_buf_match_dtag(d, CCN_DTAG_Interest)) {
		...
		interest->offset[CCN_PI_B_LastPrefixComponent] = components->buf[
				(ncomp > 0) ? (ncomp - 1) : 0];
		...
		res = ccn_parse_optional_tagged_BLOB(d, CCN_DTAG_InterestLifetime, 1,
				8);
		if (res >= 0)
			magic |= 20100401;
...
static int ccn_parse_SignedInfo(struct ccn_buf_decoder *d,
	...
	if (ccn_buf_match_dtag(d, CCN_DTAG_SignedInfo)) {
		...
		ccn_parse_required_tagged_BLOB(d, CCN_DTAG_PublisherPublicKeyDigest, 16,
				64);
		...
		/* FRDS: retrieve statistics */
		x->offset[CCN_PCO_B_ParamA] = d->decoder.token_index;
		ccn_parse_required_tagged_binary_number(d, CCN_DTAG_IntegerValue, 4, 4);
		x->offset[CCN_PCO_E_ParamA] = d->decoder.token_index;
		x->offset[CCN_PCO_B_ParamB] = d->decoder.token_index;
		ccn_parse_required_tagged_binary_number(d, CCN_DTAG_IntegerValue, 4, 4);
		x->offset[CCN_PCO_E_ParamB] = d->decoder.token_index;
		/* FRDS: end retrieve statistics */

		x->offset[CCN_PCO_B_Type] = d->decoder.token_index;
		x->type = CCN_CONTENT_DATA;
		x->type = ccn_parse_optional_tagged_binary_number(d, CCN_DTAG_Type, 3,
				3, CCN_CONTENT_DATA);
		...
		if (ccn_buf_match_dtag(d, CCN_DTAG_KeyLocator)) {
			ccn_buf_advance(d);
			x->offset[CCN_PCO_B_Key_Certificate_KeyName] =
				...
				if (ccn_parse_KeyName(d, &keyname) >= 0) {
					...
					if (keyname.PublisherID >= 0) {
						...
						x->offset[CCN_PCO_E_KeyName_Pub] =
								keyname.endPublisherID;
					}
				}
			}
			x->offset[CCN_PCO_E_Key_Certificate_KeyName] =
					d->decoder.token_index;
			ccn_buf_check_close(d);
		}
...
int ccn_ref_tagged_string(enum ccn_dtag dtag, const unsigned char *buf,
		size_t start, size_t stop, const unsigned char **presult, size_t *psize) {
	struct ccn_buf_decoder decoder;
	struct ccn_buf_decoder *d;
...
int ccn_content_get_value(const unsigned char *data, size_t data_size,
	...
	res = ccn_ref_tagged_BLOB(CCN_DTAG_Content, data,
			...
			content->offset[CCN_PCO_E_Content], value, value_size);
	return (res);
}
...
int ccn_compare_names(const unsigned char *a, size_t asize,
	...
	struct ccn_buf_decoder *aa = ccn_buf_decoder_start_at_components(&a_decoder,
			a, asize);
	struct ccn_buf_decoder *bb = ccn_buf_decoder_start_at_components(&b_decoder,
			b, bsize);
...
int ccn_parse_LinkAuthenticator(struct ccn_buf_decoder *d,
	...
	if (ccn_buf_match_dtag(d, CCN_DTAG_LinkAuthenticator)) {
		...
		pl->name_component_count = ccn_parse_optional_tagged_nonNegativeInteger(
				d, CCN_DTAG_NameComponentCount);
		...
		pl->type = ccn_parse_optional_tagged_binary_number(d, CCN_DTAG_Type, 3,
				3, CCN_CONTENT_DATA);
...
int ccn_parse_Link(struct ccn_buf_decoder *d, struct ccn_parsed_Link *link,
