...
void hashtb_end(struct hashtb_enumerator *hte) {
	...
	if (!CHECKHTE(ht, hte) || ht->refcount <= 0)
		abort();
	if (ht->refcount == 1) {
...
int hashtb_seek(struct hashtb_enumerator *hte, const void *key, size_t keysize,
		size_t extsize) {
	struct node *p = NULL;
	struct hashtb *ht = hte->ht;
	struct node **pp;
...
void hashtb_rehash(struct hashtb *ht, unsigned n_buckets) {
	...
	for (i = 0; i < ht->n_buckets; i++) {
		for (p = ht->bucket[i]; p != NULL; p = q) {
			...
			h = p->hash;
			b = h % n_buckets;
			for (pp = &bucket[b]; *pp != NULL && ((*pp)->hash < h);
					pp = &((*pp)->link))
				continue;
			p->link = *pp;
			*pp = p;
...
void** hashtb_toarray(struct hashtb *ht) {
	struct hashtb_enumerator ee;
	struct hashtb_enumerator *e = &ee;
	int i = 0;

	void** arr = calloc(ht->n, sizeof(void**));

	hashtb_start(ht, e);
	for (i = 0; e->data != NULL; ++i, hashtb_next(e)) {
		arr[i] = e->data;
	}
	hashtb_end(e);
