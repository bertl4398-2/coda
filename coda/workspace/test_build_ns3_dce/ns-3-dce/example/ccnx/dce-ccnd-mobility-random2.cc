...
static void generateTraffic(NodeContainer& nodes, CcnClientHelper& dce,
	...
	for (; iter != nodes.End(); iter++) {
		...
		if (n != c.getNodeId()) {
			...
			apps.Start(Simulator::Now() + diffPubRet + Seconds(delta*0.001 + 1.0));
			delta++;
		}
	}
...
int main(int argc, char *argv[]) {
	...
	cmd.AddValue("numPublishers",
			"Number of content producing nodes (default: 5).",
			numPublishers);
	...
	// Create FIB entries
	delta = 0;
	for (NodeContainer::Iterator iterOut = nodes.Begin();
			iterOut != nodes.End(); ++iterOut) {
		Ptr<Node> p1 = (*iterOut);
		oss.str("");
		oss << "/node" << p1->GetId();
		std::string addrSource = getAddressStringFromNode(p1);
		for (NodeContainer::Iterator iterIn = nodes.Begin();
				iterIn != nodes.End(); iterIn++) {
			Ptr<Node> p2 = (*iterIn);
			if (p1 != p2) {
				dce.SetBinary("ccndc");
				dce.ResetArguments();
	...
	for(int n = 0; n < limitN; ++n){
		for(int c = 0; c < contentPerNode; ++c){
			...
			Simulator::Schedule(Seconds(startPub), &generateTraffic, nodes, dce, cnt);
		}
	}

	// Output what we are doing
	NS_LOG_UNCOND ("Publish "<< contentPerNode <<" content items on each node; all nodes request each content " << numberRequests << " times, with an interval of " << reqInterval << "s between each request.");
	// log
	NS_LOG_UNCOND ("Nodes are initially spread in a circle with radius=" << radius << "m and move randomly.");
