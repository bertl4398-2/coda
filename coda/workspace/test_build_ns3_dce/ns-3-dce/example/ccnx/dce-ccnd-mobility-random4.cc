...
class NodeWrapper {
	...
	void selectContentRequests(std::vector<Content *>& requests) {
		...
		if (requestsPerNode > contents.size()) {
			requestsPerNode = contents.size();
		}
		/*
...
int main(int argc, char *argv[]) {
	...
	uint32_t numNodes = 50; // by default, 5x5 (25)
	...
	double interRequestPeriod = 10.0; // time to wait before issuing next request
	double width = 500.0; // place nodes in a circle of radius 1000m
	...
	uint32_t sizeBuffer = publicationsPerNode+requestsPerNode+5; // number of content items to store in the buffer
	...
	std::vector<Content*> contents;
        double txgain = 0.0;
        double rxgain = 0.0;
        std::string traceFile = "rwdata.ns_movements";
	...
	cmd.AddValue("numNodes", "Number of nodes (default: 50).", numNodes);
	cmd.AddValue("width",
			"Nodes are spread out in a square of width X meters (default: 500)",
			width);
	...
	cmd.AddValue("speed", "Node speed in m/s (default: Constant:13.89)", speed); // approximately 50km/h
        cmd.AddValue("txgain", "TxGain for the antenna", txgain);
        cmd.AddValue("rxgain", "RxGain for the antenna", rxgain);
	...
	cmd.AddValue("contentLifetime",
			"Content lifetime in seconds (default: 1000.0).", contentLifetime);
	...
	cmd.AddValue("interRequestPeriod",
			"Time (secs) to wait before issuing the next request (default: 10).",
			interRequestPeriod);
	...
	cmd.AddValue("ccnxMode",
			...
			ccnxMode);
        cmd.AddValue("trace", "Path to trace file.\n", traceFile);
	cmd.Parse(argc, argv);
	...
	oss << "speed: " << speed << std::endl;
        oss << "txgain: " << txgain << std::endl;
        oss << "rxgain: " << rxgain << std::endl;
	...
	StringValue speedSV(speed);
        ...
        if (traceFile.compare("random") == 0) {
        	MobilityHelper mobility;
	        mobility.SetPositionAllocator("ns3::RandomRectanglePositionAllocator", "X", uniformSV, "Y", uniformSV);
        	mobility.SetMobilityModel("ns3::RandomWalk2dMobilityModel", "Mode", StringValue("Time"), "Time", timeSV, "Speed", speedSV, "Bounds", boundsSV);
        	mobility.InstallAll();
        } else {
