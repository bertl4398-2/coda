...
int main(int argc, char *argv[]) {
	...
	uint32_t gridWidth = 5; // number of nodes along x axis
	...
	cmd.AddValue("gridWidth", "number of nodes along x axis", gridWidth);
	...
	mobility.SetPositionAllocator("ns3::GridPositionAllocator", "MinX",
			...
			UintegerValue(gridWidth), "LayoutType", StringValue("RowFirst"));
	...
	NS_LOG_UNCOND ("Publish content on node " << sourceNode << "; node " << sinkNode << " requests content; grid distance " << distance);
	// log
	NS_LOG_UNCOND ("Source publishes at regular intervals of '" << reqInterval << "', content expires after '" << contentLifetime << "' s.");
	// log
	NS_LOG_UNCOND ("Sink gets content at regular intervals of '" << reqInterval << "', '" << getTime-pubTime << "' s after content is published.");
